...
VebInner::VebInner(int u):
  ...
  if(u <= _veb::BASE_U) return; // base case(shouldn't reach)

  summary = (uSqrtUpper > _veb::BASE_U)
    ? static_cast<_veb*> (new VebInner(uSqrtUpper))
    : static_cast<_veb*> (new VebLeaf(uSqrtUpper));

  clusters = new _veb*[uSqrtUpper];
  if(uSqrtLower > _veb::BASE_U){
...
int VebInner::predecessor(int x) const {
  ...
  else if(mx != _veb::NIL && x > mx) return mx;
  else{
    int i = high(x, uSqrtLower), j = low(x, uSqrtLower);
    int minLow = clusters[i]->minimum();

    if(minLow != _veb::NIL && j > minLow){
      int offset = clusters[i]->predecessor(j);
      return index(i, offset, uSqrtLower);
    }
    else{
      int predCluster = summary->predecessor(i);
      if(predCluster == _veb::NIL)
        return (mi != _veb::NIL && x > mi) ? mi : _veb::NIL;
      else{
...
bool VebInner::insert(int x){
  ...
  if(u > _veb::BASE_U){ // not a base case
    int i = high(x, uSqrtLower), j = low(x, uSqrtLower);
    if(clusters[i]->minimum() == _veb::NIL)
      summary->insert(i);
...
bool VebInner::remove(int x){
  if(mi == _veb::NIL) return false; // empty set
  else if(mi == mx) // only one element
    return x != mi ? false : (mi = mx = _veb::NIL, true);
